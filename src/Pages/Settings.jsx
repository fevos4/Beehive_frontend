...
const Settings = () => {
  ...
  // Initialize hiveName from router state, fallback to "Hive 1"
  const initialHive = location.state?.hiveName || "Hive 1";
  const [ownerName, setOwnerName] = useState("Name");
  const [hiveName, setHiveName] = useState(initialHive);
  ...
  return (
    <div className="flex font-montserrat" id="settings" ref={sidebarRef}>
      <Menus isSidebarVisible={isSidebarVisible} toggleSidebar={toggleSidebar} />

      <div className={`p-10 bg-cream min-h-screen transition-all duration-300 ${
        isSidebarVisible ? "ml-0 md:ml-[250px]" : "ml-0 w-full"
      }`}>
        ...
        <p className="text-gray-600 mb-6 md:text-sm text-xs">
          Configure system parameters and notifications
        </p>

        {/* Dynamic Hive Title */}
        <h2 className="md:text-2xl text-xl font-bold mb-4">{hiveName}</h2>
